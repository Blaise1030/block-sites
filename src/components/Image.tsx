...
const ImageEditor = ({ id }: { id: string }) => {
  ...
  const { src, borderRadius, backgroundColor, link } = layout.find(({ i }) => i === id) ?.data || { link: null, src: null, borderRadius: null, backgroundColor: null };
  ...
  return <div className="p-2 h-full overflow-auto">
          ...
          <div className="flex flex-col w-full">
            <div className="m-auto mt-4 w-40 h-40 border-4 border-dashed rounded flex flex-col items-center justify-center cursor-pointer object-cover">
              <div {...getRootProps()}>
                ...
                {src && <img src={file} className="w-30 h-30 object-cover rounded" />}
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="py-2">
      ...
      <ChromePicker
        onChange={(e) => updateBackgroundColor(RGBAToHexA(e.rgb.r, e.rgb.g, e.rgb.b, e.rgb ?.a || 1))}
